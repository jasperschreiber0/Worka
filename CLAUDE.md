...
**Context building lives in `lib/project-context.ts`, shared with the intake route, not duplicated inline in chat.** `buildProjectContext()` fetches active and superseded facts as **two separate queries** — active facts get the full `MAX_FACTS_IN_PROMPT` (200, exported from `supabase/functions/smooth-responder/pipeline-logic.ts` — the same constant Stage 3/6 truncates to) ordered by confidence, superseded facts get their own small `SUPERSEDED_HISTORY_LIMIT` (20) ordered by recency. An earlier version queried both in one recency-ordered, 200-row-limited call before splitting them in JS — meaning superseded rows competed with active rows for the same budget, and an old-but-still-active, high-confidence fact could silently fall out of context purely because 200+ *newer* rows (active or superseded) had accumulated since. `selectFactsForPrompt()` (pipeline-logic.ts) does the actual truncation-with-ranking and is the **one, shared implementation** — Stage 3/6 calls it with no relevance hint (it reasons across every trade, there's no single question to be relevant to); chat calls it with categories inferred from the builder's question (`inferRelevantCategories()`, a keyword heuristic — deliberately not embeddings, see below) so a lower-confidence-but-relevant fact can be promoted ahead of a higher-confidence-but-irrelevant one once truncation actually has to choose. Change/conflict pairing (`pairSupersededFacts()`) also reuses `mergeFacts`'s own semantic-similarity check (same `SEMANTIC_DUPLICATE_THRESHOLD`, same `cosineSimilarity`) rather than only exact `category`+`key` matching — a fact superseded via embedding similarity under a different label at write time is now paired correctly at read time too, closing a gap where read-side conflict detection was strictly weaker than what write-side already guaranteed.

**Persisted understanding, not just ephemeral per-answer context.** `jobs.knowledge_confidence` / `knowledge_missing_count` / `knowledge_updated_at` (migration 035) cache the same computation `buildProjectContext()` does — written by `persistContext()`/`persistProjectUnderstanding()` (`lib/project-context.ts`) both opportunistically after every chat `project_question` answer and automatically right after intake completes (`app/api/intake/[fileId]/route.ts`, alongside the existing `ensureQuotePriced`/`runQualityAssurance` calls), so a job has a standing confidence/completeness snapshot even if nobody ever asks a chat question about it. Not yet surfaced in `JobSnapshotPanel`/morning-brief — the columns exist and are populated, wiring the UI is the natural next phase. These are a cached view over `project_facts`/`scope_items`/`clarifying_questions`, never written to directly and never authoritative on their own — recomputed and overwritten on every call, not incrementally maintained.

Logs `project_question_answered` (job_id, memory_items_loaded, documents_referenced, context_chars, confidence_score, conflicts_detected, `facts_truncated`, duration_ms) for observability — `facts_truncated` is true when a job's active fact count exceeds what fits in one prompt even after relevance-boosted ranking, not yet observed in production but logged so it's visible the moment it happens rather than silently degrading an answer.

Deliberately not built (evaluated, scope kept to what the audit showed was actually missing): a separate `project_memory` table (would duplicate `project_facts`), a second "consolidation" Claude call after every document (Stage 1/2 already does this comparison in the same call it makes today), and embeddings-based retrieval (`inferRelevantCategories()` is a coarse keyword heuristic specifically because a job's fact base is capped at 200 rows and already fits in one prompt today — real embeddings-based retrieval is justified once `facts_truncated` starts appearing routinely in production logs, not before; Voyage-embedding-based semantic dedup across documents already happens where it's needed, inside `smooth-responder`, migration 031, and is reused by `pairSupersededFacts()` rather than reimplemented).
