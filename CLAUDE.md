...
**What a real project's document set actually contains (read directly, outside the pipeline, on a stuck production job — 7 files, 16 Alfred St Woonona).** Confirms the extraction targets below are the right ones, and shows why the fact base for a project this size legitimately needs Stage 3's widened timeout (see "Anthropic failure classification" below):

- **DA/architectural drawing sets already contain a pre-computed area takeoff.** The summary sheet (`DA.A101`-equivalent) has a "Site Data Calculations" table with EXISTING vs PROPOSED GFA per floor, site coverage m²/%, FSR, and a landscape calculation — all already arithmetic, not something to re-derive from measuring the floor plan by eye. Same for the roof plan's "Roof Area Calculations" table and the window/door schedule sheets (`DA.A500`/`A501`-equivalent), which are literal BOQ rows: type, dimensions, glazing spec, location, one row per window/door tag. These should be extracted as high-confidence facts straight from the table, not re-measured from the drawing.
- **Electrical drawings encode quantities via a legend + symbol count per room per floor**, not text — downlights, GPOs, exhaust fans, wall lights, ceiling fans, TV/data points, weatherproof GPOs for outdoor areas, each with its own symbol. Getting a usable electrical quantity out of one of these is a per-symbol vision counting task, genuinely different from reading a schedule table, and worth treating as its own confidence tier (a miscounted downlight symbol is a much easier failure mode than a misread dimension).
- **Interior designer FF&E / Materials+Finishes schedules are product specifications, not BOQs — and only some rows carry real pricing.** They're organized by category with a printed subtotal (e.g. "Showers+Tapware+Waste-Basin/Floor+ $27,043.00"), one row per product with brand/model/finish/supplier-contact — but a large fraction of rows show `$0.00`/blank RRP (design-intent-only selections the client hasn't priced yet). A category subtotal is trustworthy evidence for that category's PC-allowance value; an individual `$0.00` row is not evidence of a $0 cost, it's evidence of "not yet priced" — these need to stay separate confidence signals, not be treated the same way.
- **Structural drawings are the one document type this pipeline cannot substitute for.** On the real job this note came from, the structural PDF was the one still missing, and the engine's blocking clarifying question ("provide architectural drawings for slab areas and structural systems") was correctly generated and correctly identified this exact gap — footing/slab type and structural steel sizing aren't safely inferable from an architectural set alone. Confirms Gate 1/2's unresolved-assumption behavior is doing its job here, not a bug to work around.
