...
export default function VariationCard({ variation, onApprove, onReject, onViewJob, userRole = 'owner' }: VariationCardProps) {
  ...
  const pillLabel = localStatus === 'approved' ? 'Approved' : localStatus === 'rejected' ? 'Rejected' : 'Waiting'
    ...
    >
      ...
      {variation.blocks_next_stage && (
        ...
        >
          ...
          <p className="text-[12px] font-semibold" style={{ color: 'var(--status-red)' }}>Nothing moves until this is resolved.</p>
        </div>
      )}
